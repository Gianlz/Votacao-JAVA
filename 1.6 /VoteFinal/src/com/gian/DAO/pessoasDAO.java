...
public class pessoasDAO {
    ...
    public boolean inserir(Pessoas pessoas) {
        ...
        public List<Pessoas> pesquisarTodos() {
            try {
                Connection conn = Conexao.conectar();
                String sql = "SELECT cpf, voto FROM " + NOMEDATABELA + ";";
                PreparedStatement ps = conn.prepareStatement(sql);
                ResultSet rs = ps.executeQuery();
                List<Pessoas> listObj = montarLista(rs);
                return listObj;
            } catch (Exception e) {
                //System.err.println("Erro: " + e.toString());
                //e.printStackTrace();
        ...
        public List<Pessoas> montarLista(ResultSet rs) {
            List<Pessoas> listObj = new ArrayList<Pessoas>();
            try {
                while (rs.next()) {
                    Pessoas obj = new Pessoas();
                    obj.setCpf(rs.getString(1));
                    obj.setVoto(rs.getInt(2));
                    listObj.add(obj);
                }
                return listObj;
            } catch (Exception e) {
